...
detect_env() {
    ...
    if [ -f /etc/os-release ]; then
        ...
        case "$ID" in
            ...
            arch|archarm|manjaro|endeavouros|garuda|artix) DISTRO="arch" ;;
            fedora|rhel|centos|rocky|almalinux) DISTRO="fedora" ;;
            *) DISTRO="$ID" ;;
        esac
        # Also check ID_LIKE as fallback (e.g. some Arch-based distros set ID_LIKE=arch)
        if [ "$DISTRO" = "$ID" ] && echo "${ID_LIKE:-}" | grep -q "arch"; then
            DISTRO="arch"
        fi
        if [ "$DISTRO" = "$ID" ] && echo "${ID_LIKE:-}" | grep -q "debian\|ubuntu"; then
            DISTRO="ubuntu"
        fi
...
ensure_deps() {
    ...
    case "$DISTRO" in
        ...
        arch)
            ...
            # Arch ARM: also ensure unzip for bun installer
...
create_launcher() {
    ...
    # Follow 302 redirect (GitHub releases → S3), 15s connect timeout, 120s max
    if curl -fsSL -L --connect-timeout 15 --max-time 120 "$bin_url" -o /tmp/rin-bin.tar.gz 2>/dev/null; then
